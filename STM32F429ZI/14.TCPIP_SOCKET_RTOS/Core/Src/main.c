...
#include "lwip/udp.h"  				// ADD_Hyuckku
#include "udp_echoserver.h"  	// ADD_Hyuckku
...
char temp_str[40];		// ADD temp buff
...
void StartDefaultTask(void *argument)
{
  ...
  IP4_ADDR(&addr1, 10,10,15,57);	// client PC IP address
  ...
  for(;;)
  {
	...
	sprintf(temp_str, "hello Hyuckku!!\n");
