...
volatile unsigned char rx_buff[COMMAND_SU][COMMAND_LENGTH];
...
void pc_command_processing(void)
{
	if(newline_detect_flag)	// comport master로부터 완전한 문장이 들어오면(\n을 만나면)
	{
		...
		printf("%s\n", rx_buff);
		if(!strncmp(rx_buff, "led_all_on", strlen("led_all_on")))	// if(!strncmp(rx_buff, "ledallon", strlen("ledallon")))==1
		{
			led_all_on();
			return;
		}
		if(!strncmp(rx_buff, "led_all_off", strlen("led_all_off")))	// if(!strncmp(rx_buff, "ledallon", strlen("ledallon")))==1
		{
			led_all_off();
			return;
		}
