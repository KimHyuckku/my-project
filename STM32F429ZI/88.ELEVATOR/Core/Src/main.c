...
extern int dotmatrix_main_stop(void);

extern void set_rpm(int rpm);
extern void stepmotor_running(void);
extern void setpmotor_drive(int step);
...
// 1. moved from Driver/STM32F4xx_HAL_Driver/Src/stm32f4xx_hal_gpio.c to here
// 2. enter here
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	case PHOTO1_Pin:
		...
		current_elevator_floor = 2;
		break;
	case PHOTO2_Pin:
		ext_int_no = GPIO_Pin;
		HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_14);	// LED3
		current_elevator_floor = 3;
		break;
	case PHOTO3_Pin:
		ext_int_no = GPIO_Pin;
		HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_0);	//LED1
		current_elevator_floor = 4;
		break;
	}
}
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pins : BUTTON4F_Pin BUTTON3F_Pin BUTTON2F_Pin BUTTON1F_Pin */
  GPIO_InitStruct.Pin = BUTTON4F_Pin|BUTTON3F_Pin|BUTTON2F_Pin|BUTTON1F_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);
...
void Task2_func(void *argument)
{
  ...
  for(;;)
  {
	  if(dir == 1) dotmatrix_main_up();
	  else if(dir ==2) dotmatrix_main_down();
	  else if(dir == 0)
	  {
		  dotmatrix_main_stop();
	  }
