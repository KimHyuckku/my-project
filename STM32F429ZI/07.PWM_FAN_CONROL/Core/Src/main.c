...
/* USER CODE BEGIN PV */
uint8_t rx_data;	// uart3 rx_data
uint8_t bt_rx_data;	// uart6 rx_data
...
/* USER CODE BEGIN 0 */
//---------------------
// call by SysTic_Handler of stm32f4xx_it.c
// ARM default timer
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	...
	{
		TIM10_10ms_counter++;	// 10 ms timer counter
...
int main(void)
{
  ...
  HAL_UART_Receive_IT(&huart6, &bt_rx_data, 1);		//assing to BT RX INT
  HAL_TIM_Base_Start_IT(&htim10);		// ADD_HYUCK_1011
  HAL_TIM_Base_Start_IT(&htim11);		// ADD_HYUCK_1011
  HAL_TIM_IC_Start_IT(&htim3, TIM_CHANNEL_1);	// for count pulse(rising edge & falling edge) - echo
...
//  servo_motor_test_main();
  ...
  while (1)
  {
	  ...
	  dcmotor_pwm_control();
	  /*
	  if(TIM10_10ms_counter == 50) //&& TIM10_10ms_counter<10)
	  {
		 HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_0);	// LED1
	  }
	  else if(TIM10_10ms_counter == 100)// && TIM10_10ms_counter<15)
	  {
		  HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_0);
		  HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_7);
	  }
	  else if(TIM10_10ms_counter == 150)// && TIM10_10ms_counter<20)
	  {
		  HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_0);
		  HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_14);
	  }
	  else if(TIM10_10ms_counter == 200) //&& TIM10_10ms_counter<25)
	  {
		  HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_0);
		  HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_7);
	  }
	  else if(TIM10_10ms_counter == 250) //&& TIM10_10ms_counter<30)
	  {
		  HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_0);

	  }
	  else if(TIM10_10ms_counter >= 300 )
	  {
		  HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_0);
		  HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_7);
		  HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_14);
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOE, IN1_DCMOTOR_Pin|IN2_DCMOTOR_Pin|FAN_FORWARD_BACKWARD_LED_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : IN1_DCMOTOR_Pin IN2_DCMOTOR_Pin FAN_FORWARD_BACKWARD_LED_Pin */
  GPIO_InitStruct.Pin = IN1_DCMOTOR_Pin|IN2_DCMOTOR_Pin|FAN_FORWARD_BACKWARD_LED_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOE, &GPIO_InitStruct);

  /*Configure GPIO pins : BUTTON3_Pin BUTTON5_Pin BUTTON2_Pin BUTTON1_Pin
                           BUTTON0_Pin */
  GPIO_InitStruct.Pin = BUTTON3_Pin|BUTTON5_Pin|BUTTON2_Pin|BUTTON1_Pin
                          |BUTTON0_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOE, &GPIO_InitStruct);
