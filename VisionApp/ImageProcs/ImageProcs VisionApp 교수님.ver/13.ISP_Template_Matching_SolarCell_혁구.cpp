...
void main()
{
	...
	for (size_t img_index = 1; img_index <= img_counts; img_index++)
	{
		...
		double spec_height_max = avg_height + avg_height * per + 1;
		...
		for (size_t i = 0; i < contours.size(); i++)
		{
			...
			if (contours_height > spec_height_min * 2 && contours_height <= spec_height_max * 2)
			{
				...
				for (size_t col = 1; col < subRegion.cols - 1; col++)
				{
					...
					for (size_t row = avg_height - 2; row <= avg_height + 2; row++)
					{
						int index = row * subRegion.cols + col;
						int value = subDataBin[index];
						sum += value;
					}
					if (sum < hole_sz)
					{
				...
				int radius = 7;
				Point cir_ctr = Point(rrt.boundingRect().tl().x + hole_right - hole_length / 2, rrt.boundingRect().tl().y + avg_height);
