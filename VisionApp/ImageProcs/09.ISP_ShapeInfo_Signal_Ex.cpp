...
int main()
{
	...
	// opening
	Mat src_open, src_closing, src_oepn_close, src_close_open, src_open_open;
	int type = MorphTypes::MORPH_OPEN;
	cv::morphologyEx(src_hsv_bin, src_open, type, SE);
	Mat diff_open = src_hsv_bin - src_open;

	type = MorphTypes::MORPH_CLOSE;
	cv::morphologyEx(src_hsv_bin, src_closing, type, SE);
	Mat diff_close = src_hsv_bin - src_closing;

	cv::morphologyEx(src_open, src_oepn_close, type, SE);

	type = MorphTypes::MORPH_OPEN;
	cv::morphologyEx(src_closing, src_close_open, type, SE);

	kernerSz = 1;
	shape = MorphShapes::MORPH_RECT;
	sz = Size(2 * kernerSz + 1, 2 * kernerSz + 1);
	cv::morphologyEx(src_open, src_open_open, type, SE);
