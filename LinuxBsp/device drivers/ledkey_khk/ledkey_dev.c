...
void kerneltimer_func(struct timer_list *t)
{
//	keyled_data * pKeyLedData = (keyled_data *)kmalloc(sizeof(keyled_data), GFP_KERNEL);
//	keyled_data * pKeyLedData = (keyled_data *)t
	keyled_data info;
#if DEBUG
    printk("ledVal : %#04x\n", (unsigned int)(ledVal));
#endif
...
//			wait_event_interruptible_timeout(WaitQueue_Read, pKeyData->keyNumber,100); // 100 * 1/HZ = 100 * 1/100 = 1 Sec 
		}
	}
...
__poll_t ledkey_poll(struct file *filp, struct poll_table_struct * wait)
{
	unsigned int mask=0;
    keyData * pKeyData = (keyData *)filp->private_data;
#ifdef DEBUG
...
//	keyled_data ctrl_info = {0};
    int err=0;
//	keyled_data * pKeyLedData = (keyled_data *)filp->private_data;
	keyled_data info;
	switch( cmd )
    {
        case TIMER_START :
