...
void CDlgTestView::OnIddDialog()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		strOutText.Format(_T("Name: %s, Age: %d\n Licence : %d "),
			dlg.m_strName, dlg.m_nAge, dlg.m_bLicence);
		strOutText2.Format(_T("Job: %d, Residence : %d, Licence: %d"), dlg.m_nJob, dlg.m_nResidence, dlg.m_bLicence);
...
void CDlgTestView::OnIddDlg2()
{
	...
	{
		temp.Format(_T("컨트롤키가(%d)로 눌러옴"), dlg2.m_ctrlCheck);
		MessageBox(temp);
	}
